# Job Application Optimizer — Gemini CLI Instructions
...
```
jobs.xlsx  →  (Gemini reads descriptions)  →  score + tailor  →  resumes/*.pdf
```
...
2. **Gemini** — reads each row, scores fit, tailors the resume for matches, writes PDFs to `resumes/`
...
If it fails: `pip3 install openpyxl`
...
```
If missing: `brew install --cask mactex-no-gui`

**If using `resume.docx`:**
```bash
...
Include:
- `CLAUDE.md`, `GEMINI.md`, `resume_optimizer_skill.md`, `linkedin-job-tracker/`
...
New users: add their own resume (`.tex` or `.docx`) and `jobs.xlsx`, then open the folder in Gemini CLI.
